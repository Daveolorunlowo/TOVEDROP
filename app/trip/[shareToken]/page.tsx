...
export default async function TripSharePage({ params }: { params: { shareToken: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: '#111111' }}>
        <div className="w-full max-w-sm text-center">
          <div className="w-16 h-16 rounded-full mx-auto mb-6 flex items-center justify-center" style={{ background: '#1a1a1a', border: '1px solid #222' }}>
            <ShieldCheck className="w-8 h-8" style={{ color: '#555' }} />
          </div>
          <h1 className="text-lg font-semibold mb-2" style={{ color: '#f5f5f5' }}>Link Inactive</h1>
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-10 flex flex-col items-center justify-center" style={{ background: '#111111' }}>
      <div className="w-full max-w-md">
        ...
        {/* Main Card */}
        <div className="rounded-2xl overflow-hidden mb-6" style={{ background: '#171717', border: '1px solid #222' }}>
          
          {/* Driver Info */}
          <div className="p-5" style={{ borderBottom: '1px solid #1e1e1e' }}>
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-4" style={{ color: '#555' }}>Driver & Vehicle Details</p>
            <div className="flex items-center gap-4">
              <Avatar className="w-12 h-12 shrink-0 border" style={{ borderColor: '#222' }}>
                <AvatarFallback className="text-sm font-bold" style={{ background: '#111', color: '#f5f5f5' }}>
                  {driverInitials}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <h2 className="text-base font-bold truncate" style={{ color: '#f5f5f5' }}>
            ...
            {/* Vehicle Details */}
            {driverProfile && (
              <div className="mt-5 rounded-lg p-3.5 flex items-center gap-3" style={{ background: '#111111', border: '1px solid #1e1e1e' }}>
                <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: '#171717', border: '1px solid #222' }}>
                  <Car className="w-4 h-4" style={{ color: '#888' }} />
                </div>
          ...
          {/* Route Info */}
          <div className="p-5">
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-4" style={{ color: '#555' }}>Route & Time</p>
            <div className="relative pl-6 space-y-4">
              {/* Vertical line connecting dots */}
              <div className="absolute top-2.5 bottom-2.5 left-2.5 w-[2px] rounded-full" style={{ background: '#1e1e1e' }} />
              
              <div className="relative">
                <div className="absolute -left-6 top-1 w-2.5 h-2.5 rounded-full" style={{ background: '#333' }} />
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: '#555' }}>Pickup</p>
                <p className="text-sm font-medium" style={{ color: '#f5f5f5' }}>{trip?.pickup}</p>
              </div>
              
              <div className="relative">
                <div className="absolute -left-6 top-1 w-2.5 h-2.5 rounded-full" style={{ background: 'var(--orange-brand)' }} />
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: '#555' }}>Destination</p>
                <p className="text-sm font-medium" style={{ color: '#f5f5f5' }}>{trip?.destination}</p>
              </div>
            </div>

            <div className="mt-5 pt-4 flex items-center justify-between" style={{ borderTop: '1px solid #1e1e1e' }}>
              <div className="flex items-center gap-2">
                <Calendar className="w-3.5 h-3.5" style={{ color: '#555' }} />
                <span className="text-xs font-medium" style={{ color: '#888' }}>{trip?.date}</span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="w-3.5 h-3.5" style={{ color: '#555' }} />
                <span className="text-xs font-medium" style={{ color: '#888' }}>{trip?.time}</span>
              </div>
            </div>
          </div>
        ...
        {/* Footer Note */}
        <p className="text-[10px] text-center px-4" style={{ color: '#555', lineHeight: '1.6' }}>
