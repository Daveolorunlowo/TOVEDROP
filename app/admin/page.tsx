...
function AnimatedNumber({ value, isCurrency = false }: { value: number, isCurrency?: boolean }) {
  ...
  return <span className="font-mono tracking-tight">{isCurrency ? `₦${formatted}` : formatted}</span>
}
...
const Card = ({ children, className }: { children: React.ReactNode, className?: string }) => (
  <div className={cn("bg-[#14151a] border border-white/[0.04] rounded-xl overflow-hidden", className)}>
    {children}
  </div>
)
...
const Button = ({ children, variant = 'primary', className, ...props }: any) => {
  ...
  const variants = {
    ...
    secondary: "bg-transparent border border-white/10 text-white hover:bg-[#1c1d24] active:scale-[0.98]"
  }
  return (
...
const EmptyState = ({ icon: Icon, title, desc, action }: any) => (
  <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
    ...
    <p className="text-xs text-white/50 mb-6 max-w-xs">{desc}</p>
    {action}
  </div>
)
...
const ListRow = ({ children, status, className }: any) => {
  ...
  return (
    <div className={cn("flex items-center p-4 border-b border-white/5 bg-[#14151a] hover:bg-[#1c1d24] transition-colors border-l-[3px]", borderColor, className)}>
      {children}
    </div>
  )
}
...
export default function AdminDashboardPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0e0e11] text-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs font-medium text-white/50 uppercase tracking-widest">Loading Dashboard</p>
        </div>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <Card key={i} className="p-6 flex flex-col justify-between">
            <p className="text-xs font-medium text-white/50 uppercase tracking-widest mb-4">{s.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            ...
            <select className="bg-transparent border border-white/10 rounded-md text-xs text-white/70 px-3 py-1.5 focus:outline-none focus:border-white/30">
          ...
          <div className="flex-1 min-h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#14151a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '12px', color: 'white' }}
                ...
                {recentActivity.map((act: any) => (
                  <ListRow key={act.id} status={act.type === 'TRIP' ? 'completed' : 'processing'} className="px-6 border-l-[3px]">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-white/50 truncate mt-0.5">{act.desc}</p>
                    </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">Manage registered platform users.</p>
        </div>
        <div className="relative">
          ...
          <input 
            ...
            className="pl-9 pr-4 py-2 bg-[#14151a] border border-white/10 rounded-lg text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 w-full sm:w-64"
          />
        </div>
      </div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-[#1c1d24] text-[10px] uppercase font-semibold text-white/50 tracking-widest border-b border-white/5">
              <tr>
            ...
            <tbody className="divide-y divide-white/5">
              {users.filter((u:any) => u.name?.toLowerCase().includes(search.toLowerCase()) || u.email?.toLowerCase().includes(search.toLowerCase())).map((u: any) => (
                <tr key={u.id} className="hover:bg-[#1c1d24] transition-colors">
                  <td className="px-6 py-4 font-medium text-white">{u.name}</td>
                  <td className="px-6 py-4 text-white/70">{u.email}</td>
                  <td className="px-6 py-4 text-right text-white/50 font-mono">{new Date(u.createdAt).toLocaleDateString()}</td>
                </tr>
              ))}
              {users.length === 0 && (
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">Manage driver applications and active fleet.</p>
        </div>
        <div className="relative">
          <Search className="w-4 h-4 text-white/30 absolute left-3 top-1/2 -translate-y-1/2" />
          <input 
        ...
        <Card className="border-amber-500/20">
          ...
          <div className="divide-y divide-white/5">
            {pendingDrivers.filter((d: any) => d.user.name?.toLowerCase().includes(search.toLowerCase())).map((driver: any) => (
              <div key={driver.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-[#1c1d24] transition-colors">
                <div className="flex items-center gap-4">
                  <Avatar className="w-10 h-10 border border-white/10">
                    <AvatarFallback className="bg-white/5 text-white text-xs font-semibold">{driver.user.name?.slice(0,2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="text-base font-semibold text-white">{driver.user.name}</p>
                    <p className="text-xs text-white/50 mt-0.5">{driver.user.email}</p>
                  </div>
                </div>
                <div className="flex flex-col md:items-end gap-1">
                  <p className="text-sm text-white font-medium">{driver.carModel || 'Unknown Vehicle'}</p>
                  <p className="text-xs text-white/50 font-mono">{driver.plateNumber || 'NO PLATE'}</p>
                </div>
      ...
      <Card>
         ...
         <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-white/5">
               ...
               <tr key={driver.id} className="hover:bg-[#1c1d24] transition-colors">
                  <td className="px-6 py-4">
                     <p className="font-semibold text-white text-sm">{driver.user.name}</p>
                     <p className="text-xs text-white/50 mt-0.5">{driver.user.email}</p>
                  </td>
                  <td className="px-6 py-4">
                     <p className="text-white text-sm">{driver.carModel}</p>
                     <p className="text-white/50 text-xs font-mono mt-0.5">{driver.plateNumber}</p>
                  </td>
  ...
  const renderFinances = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
        <div>
          <h2 className="text-2xl font-semibold text-white">Finances & Payouts</h2>
          <p className="text-sm text-white/50 mt-1">Manage revenue and driver withdrawals.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <Card className="p-6">
             <p className="text-xs font-medium text-white/50 uppercase tracking-widest mb-2">Total System Revenue</p>
             <div className="text-3xl font-semibold text-white">
                <AnimatedNumber value={data?.stats?.totalRevenue || 0} isCurrency />
             </div>
           </Card>
           <Card className="p-6">
             <p className="text-xs font-medium text-white/50 uppercase tracking-widest mb-2">Total Driver Payouts</p>
             <div className="text-3xl font-semibold text-white">
        ...
        <Card>
          ...
          {pending.length === 0 ? (
            ...
            <div className="divide-y divide-white/5">
              {pending.map((req: any) => (
                <div key={req.id} className="p-6 flex flex-col sm:flex-row items-center justify-between gap-6 hover:bg-[#1c1d24] transition-colors border-l-[3px] border-l-amber-500">
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="text-base font-semibold text-white">{req.driver?.user?.firstName} {req.driver?.user?.lastName}</p>
                      <p className="text-xs text-white/50 mt-0.5">{req.driver?.user?.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-8">
                    <p className="text-xl font-semibold text-white font-mono">₦{req.amount.toLocaleString()}</p>
  ...
  const renderSecurity = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div>
        <h2 className="text-2xl font-semibold text-white">Security & Logs</h2>
        <p className="text-sm text-white/50 mt-1">System monitoring and anomaly detection.</p>
      </div>
            ...
            {securityLogs.map((log: any) => (
              <ListRow key={log.id} status={log.success ? 'success' : 'failed'}>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-white">{log.success ? 'Authentication Success' : 'Failed Login Attempt'}</p>
                  <p className="text-xs text-white/50 mt-1">Target: {log.email} • IP: <span className="font-mono text-white/70">{log.ipAddress || 'UNKNOWN'}</span></p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-white/40 font-mono">{new Date(log.createdAt).toLocaleString()}</p>
                </div>
              </ListRow>
            ))}
          </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#0e0e11] text-white font-sans selection:bg-[#F97316]/30">
      {/* Sidebar */}
      <aside className={cn(
        'fixed inset-y-0 left-0 z-50 flex flex-col w-[260px] bg-[#0e0e11] border-r border-white/5 transition-transform duration-300 lg:static lg:translate-x-0',
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-1 px-4 [&::-webkit-scrollbar]:hidden">
          <p className="px-4 text-[10px] font-semibold text-white/30 uppercase tracking-widest mb-2 mt-4">Menu</p>
          {NAV_ITEMS.map(item => {
            ...
            return (
              ...
              >
                <item.icon className={cn("w-4 h-4 transition-colors", active ? "text-[#F97316]" : "text-white/40 group-hover:text-white/80")} />
                {item.label}
              </button>
            )
          })}
        ...
        <div className="p-4 border-t border-white/5 shrink-0">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-white/50 hover:bg-white/5 hover:text-white text-sm font-medium transition-colors"
          >
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {/* Top Header */}
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-white/5 shrink-0 bg-[#0e0e11]/80 backdrop-blur-md z-40">
          <div className="flex items-center gap-4">
            <button className="lg:hidden p-2 rounded-lg text-white/50 hover:bg-white/5 hover:text-white transition-colors -ml-2" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-lg border border-white/5">
                <Avatar className="w-6 h-6 border border-white/10">
                   <AvatarFallback className="bg-[#1c1d24] text-[10px] text-white">AD</AvatarFallback>
                </Avatar>
        ...
        {/* Dynamic Views */}
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 w-full [&::-webkit-scrollbar]:hidden bg-[#0e0e11]">
