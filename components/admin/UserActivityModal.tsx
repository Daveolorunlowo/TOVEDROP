...
export function UserActivityModal({ user, onClose }: { user: any, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[#111111] border border-[#222] rounded-xl w-full max-w-2xl max-h-[85vh] shadow-2xl flex flex-col animate-fade-in-up">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[#222]">
          <div>
            ...
            <p className="text-xs text-[#888] mt-0.5">Viewing history for <span className="text-white font-medium">{user.name}</span> ({user.email})</p>
          </div>
        ...
        {/* Tabs */}
        <div className="flex items-center border-b border-[#222] px-4 gap-6 pt-2 bg-[#171717]">
          {(['trips', 'drops', 'feedbacks'] as const).map(tab => (
            <button
              ...
              className={`pb-3 text-xs font-semibold uppercase tracking-wider transition-colors relative ${
                activeTab === tab ? 'text-orange-brand' : 'text-[#666] hover:text-[#aaa]'
              }`}
            >
              {tab}
              {activeTab === tab && (
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-[#111]">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-[#888] gap-2">
            ...
            <div className="space-y-3">
              {activeTab === 'trips' && (
                data.trips.length === 0 ? (
                  ...
                  data.trips.map(trip => (
                    <div key={trip.id} className="p-3 bg-[#171717] rounded-lg border border-[#222] flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[#888] mt-0.5 truncate max-w-[200px] sm:max-w-[300px]">
                            {trip.pickupLocation} <span className="text-[#444] mx-1">→</span> {trip.dropoffLocation}
                          </p>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-xs font-medium px-2 py-0.5 rounded bg-[#1e1e1e] text-[#aaa] inline-block mb-1">
                          {trip.status}
                        </p>
              ...
              {activeTab === 'drops' && (
                data.drops.length === 0 ? (
                  ...
                  data.drops.map(drop => (
                    <div key={drop.id} className="p-3 bg-[#171717] rounded-lg border border-[#222] flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-white">{drop.type}</p>
                          <p className="text-xs text-[#888] mt-0.5">{drop.description || 'No details provided'}</p>
                        </div>
                      </div>
              ...
              {activeTab === 'feedbacks' && (
                data.feedbacks.length === 0 ? (
                  ...
                  data.feedbacks.map(feedback => (
                    <div key={feedback.id} className="p-3 bg-[#171717] rounded-lg border border-[#222]">
                      <div className="flex items-center gap-2 mb-2">
                        <MessageSquare className="w-4 h-4 text-orange-brand" />
                        <span className="text-xs font-bold text-[#888] uppercase tracking-wide">{feedback.category}</span>
